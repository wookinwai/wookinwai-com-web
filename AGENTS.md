...
**Static Astro 7 site.** Fully prerendered, no SSR and no Cloudflare adapter. `astro.config.mjs` sets `site`, the sitemap integration, and the Tailwind Vite plugin, and output is plain static assets in `dist/`. Default `compressHTML` is `'jsx'` (Astro 7): whitespace that falls across a newline between inline elements is dropped, so intended spaces at such seams are written explicitly with `{' '}` (see `Footer.astro`).
